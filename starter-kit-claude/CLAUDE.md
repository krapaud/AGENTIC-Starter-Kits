...
Si le message utilisateur commence par `Mode initialisation :`, suspendre le flux courant et exécuter la procédure d’initialisation complète, même dans une conversation déjà ouverte. Relire `CLAUDE.md`, `.claude/START-HERE.md`, `.claude/CONVERSATION-MODES.md`, `.claude/PROJECT-BRIEF.md`, `.claude/project-profile.toml` et `.claude/RUNTIME-STATE.md` s’ils existent. Vérifier le statut du cahier des charges. S’il est absent ou `pending`, appliquer uniquement `project-intake` et demander le cahier sans analyser la stack, créer de work item ou modifier le produit. Ne reprendre l’orchestration qu’après acceptation du cahier et validation de la phase de complétude.
Lire aussi .claude/policies/INITIALIZATION-TRANSPARENCY.md. Afficher obligatoirement l origine du cahier, son statut, les fichiers réellement lus, les fichiers manquants et l action suivante. Si le cahier est present et accepted, dire explicitement qu il est reutilise et ne sera pas remplace.
...
Lire et appliquer `.claude/policies/CORE-EXECUTION-CONTRACT.md` avant toute orchestration. Construire le registre des obligations, évaluer toutes les portes à chaque transition et interdire toute clôture tant qu une obligation applicable n est pas prouvée.

Appliquer `.claude/policies/EXECUTION-MODE-POLICY.md`. Pour toute construction, utiliser le mode Agent avec Work locally. Le mode Plan et la lecture seule ne permettent pas de poursuivre une carte.
...
Lire et appliquer `.claude/policies/SESSION-CONTINUITY-POLICY.md`. Pour une tâche multi-étapes, activer un objectif persistant natif de l'environnement lorsqu'il existe et renseigner ses critères dans `RUNTIME-STATE.md`. Sans cette capacité, conserver l'état local et ne jamais promettre une continuation automatique après la fin du tour.

Pour une demande de finalisation d'une carte Trello comportant au moins deux cases ouvertes, le Coordinateur doit initialiser ce Goal automatiquement avant la première case.
...
2. Le chemin exact du fichier vérifié : .claude/PROJECT-BRIEF.md.
...
Si le projet contient .workspace.toml et .claude/scripts/sync-workspace-kit.sh, exécuter ce synchroniseur au début d une nouvelle session avant toute analyse. Il vérifie la version distante, sauvegarde le kit local et ne remplace jamais le cahier des charges, le profil projet, l état d exécution, les décisions, les work items ou les journaux. Si la mise à jour échoue, conserver la version actuelle et poursuivre uniquement avec le kit local après signalement.
